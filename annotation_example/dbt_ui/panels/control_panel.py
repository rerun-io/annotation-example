...
class ControlPanel:
    ...
    run_calibrate: gr.Button | None = None
    ...
    def build(self):
        with gr.Column() as self.root:
            ...
            with gr.Row():
                self.run_bbox = gr.Button("Run Hand Detection on all Frames")
                self.run_calibrate = gr.Button("Run Multiview Calibration")
        return self
    ...
    def wire(
        ...
        # )
        self.run_bbox.click(
            fn=ctrl.engine.predict_mv_xyxy,
            inputs=[state],
            outputs=[viewer, state],
        )

        self.run_calibrate.click(
            fn=ctrl.log_calibration_results,
            inputs=[state],
            outputs=[viewer, state],
        )
